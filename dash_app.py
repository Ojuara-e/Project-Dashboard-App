# Projeto 5: Data App Para Dashboard Interativo de Sales Analytics em Python com Streamlit
...
st.set_page_config(
    page_title="Dashboard App",  # Título que aparece na aba do navegador
...
def dash_cria_conexao(db_path = "dash_database.db"):
...
def dash_filtros_sidebar(df):

    """
    Cria todos os widgets da sidebar (menu lateral).
    1. Exibe o banner da aplicação.
    ...
    st.sidebar.markdown(
        ...
        <div style="background-color:#00CC96; padding: 10px; border-radius: 5px; text-align: center; margin-bottom: 15px;">
            <h3 style="color:white; margin:0; font-weight:bold;">Empresa Fulinha Ltda</h3>
        </div>
        """,
    ...
    with st.sidebar.expander("🆘 Suporte / Fale conosco", expanded = False):
        st.write("Se tiver dúvidas envie mensagem para ellihasdhavisaraujo@gmail.com")
    
    # Adiciona uma legenda de rodapé com 'st.sidebar.caption'
    st.sidebar.caption("Aplicativo Desenvolvido por Ellihas Freitas.")

    # Retorna o DataFrame recém-filtrado para ser usado no corpo principal da página
    return df_dash_filtrado


# --- Bloco 6: Função para Renderizar os Cards de KPIs ---

# Função para os KPIs
def dash_renderiza_cards_kpis(df):
...
def dash_gera_pdf_report(df_dash_filtrado, total_faturamento, total_quantidade, avg_ticket):

    """
    Gera um relatório PDF customizado usando a biblioteca FPDF.
    
    Parâmetros:
    df_dash_filtrado (pd.DataFrame): O DataFrame filtrado.
    ...
    df_top = df_dash_filtrado.sort_values("faturamento", ascending=False).head(15)
...
def dash_project():
    ...
    df = dash_carrega_dados()
    
    # Chama a função (Bloco 5) que desenha a sidebar e retorna
    # o DataFrame já filtrado (df_dash_filtrado) com base nas seleções do usuário.
    df_dash_filtrado = dash_filtros_sidebar(df)

    # --- Início: Layout da Página Principal ---
    
    # Define os títulos e a descrição que aparecem no corpo principal do app
    st.title("Aplicativo para Sales Analytics")
    st.title("📊 Dashboard Interativo de Sales Analytics")
    ...
    total_faturamento, total_qty, avg_ticket = dash_renderiza_cards_kpis(df_dash_filtrado)
    ...
    with tab1:
        ...
        with col_left:
            ...
            daily_rev = df_dash_filtrado.groupby("date")[["faturamento"]].sum().reset_index()
        ...
        with col_right:
            ...
            cat_rev = df_dash_filtrado.groupby("categoria")[["faturamento"]].sum().reset_index()
        ...
        with c_a:
            ...
            fig_bar = px.bar(
                df_dash_filtrado.groupby("regiao")[["faturamento"]].sum().reset_index(),
        ...
        with c_b:
            ...
            df_dash_filtrado["weekday_num"] = df_dash_filtrado["date"].dt.dayofweek
            
            # .map() usa o dicionário para "traduzir" os números
            df_dash_filtrado["dia_semana"] = df_dash_filtrado["weekday_num"].map(dias_pt_map)

            # Agrupa pelo nome em PT, calcula a média, e reordena
            wd_rev = df_dash_filtrado.groupby("dia_semana")[["faturamento"]].mean().reindex(dias_pt_ordem).reset_index()
        ...
        fig_scat = px.scatter(
            df_dash_filtrado, x="quantidade", y="faturamento", color="categoria", size="faturamento",
    ...
    with tab2:
        ...
        st.dataframe(df_dash_filtrado, width='stretch', height=400) 
        ...
        with c_exp1:
            ...
            csv = df_dash_filtrado.to_csv(index=False).encode('utf-8')
        ...
        with c_exp2:
            ...
            if st.button("📄 Gerar Relatório PDF", width='stretch'): 
                ...
                with st.spinner("Renderizando PDF..."):
                    ...
                    pdf_bytes = dash_gera_pdf_report(df_dash_filtrado, total_faturamento, total_qty, avg_ticket)
    ...
    with st.expander("ℹ️ Sobre Esta Data App", expanded=False):
        ...
        **Recursos Integrados:**
        ...
        - **OBS:** Para conhecer mais sobre o Ellihas visite: https://www.linkedin.com/in/elias-a-freitas/.
        """)
...
# Se você executar este script DIRETAMENTE (ex: 'python dash_app.py'),
# Python definirá __name__ = "__main__".
# Se este script for IMPORTADO por outro script,
# Python definirá __name__ = "dash_app" (o nome do arquivo).

# Portanto, este 'if' garante que o código dentro dele (a chamada para 'dash_project()')
